...
def easyGame():
    ...
    while inGame:
        ...
        gameWin.blit(winBg, (0,0))
        text_surface = my_font.render('Green = Z | Blue = Q | Red = S | White = D', False, (0, 0, 0))
        gameWin.blit(text_surface, (0, 0))
        pygame.display.flip()
        ...
        for i in range(50):
            ...
            colorlist = [(GREEN, K_z), (BLUE, K_q), (RED, K_s), (WHITE, K_d)]
            
            choosenCombo = random.choice(colorlist)
            choosencolor = choosenCombo[0]
            goodKey = choosenCombo[1]

            pygame.draw.circle(gameWin, choosencolor, (x,y), 40, 40)
            pygame.display.flip()
            ...
            gameWin.blit(winBg, (0,0))
            text_surface = my_font.render('Green = Z | Blue = Q | Red = S | White = D | ' + str(i+1) + '/50', False, (0, 0, 0))
            gameWin.blit(text_surface, (0, 0))
            pygame.display.flip()
        ...
        if float(bestscoreData['easyMode']) > float(averageReactTime):
            newscoretext = my_font.render('Brand new record !!! gg sir :)', False, RED)
            gameWin.blit(newscoretext, (250, 200))
            bestscoreData['easyMode'] = float(averageReactTime)
